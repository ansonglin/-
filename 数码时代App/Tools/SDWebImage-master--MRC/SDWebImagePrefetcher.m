...
- (void)reportStatus
{
    ...
    NSLog(@"Finished prefetching (%ld successful, %ld skipped, timeElasped %.2f)",(unsigned long) total - _skippedCount, (unsigned long)_skippedCount, CFAbsoluteTimeGetCurrent() - _startedTime);
}
...
{
    _finishedCount++;
    NSLog(@"Prefetched %ld out of %ld", (unsigned long)_finishedCount, (unsigned long)[self.prefetchURLs count]);
...
{
    _finishedCount++;
    NSLog(@"Prefetched %ld out of %ld (Failed)", (unsigned long)_finishedCount,(unsigned long) [self.prefetchURLs count]);
